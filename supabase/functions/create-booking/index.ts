...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // 6. Round-robin: sort available agents by priority, then pick next in rotation
    ...
    const priorityMap = Object.fromEntries(
      (priorities ?? []).map((r: { agent_id: string; priority: number | null }) => [r.agent_id, r.priority ?? 10])
    )

    // Sort by priority ascending (lower = higher priority), then stable by id for tie-breaking
    const sortedAvailable = [...availableAgents].sort((a, b) =>
      (priorityMap[a.id] ?? 10) - (priorityMap[b.id] ?? 10)
    )
    ...
    const assignedAgent = roundRobinSelect(sortedAvailable, lastAgentId)
    ...
    // 8. Create Google Calendar event with Meet link
...
async function createGoogleCalendarEvent(opts: {
  ...
  const resp = await fetch(
    `https://www.googleapis.com/calendar/v3/calendars/${encodeURIComponent(opts.calendarId)}/events?sendNotifications=true&conferenceDataVersion=1`,
    {
      method: 'POST',
  ...
  const eventData = await resp.json() as {
    id: string
    conferenceData?: {
  ...
  return { id: eventData.id, meetLink }
}
