...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // 1b. Enforce booking window constraints
    const minNoticeHours: number = (link as { min_notice_hours?: number }).min_notice_hours ?? 24
    const maxAdvanceDays: number = (link as { max_advance_days?: number }).max_advance_days  ?? 14

    const requestedDateObj = new Date(`${date}T00:00:00`)
    // Reject if date is beyond the max advance window
    ...
    const minNoticeCutoff = new Date(Date.now() + minNoticeHours * 60 * 60 * 1000)

    let cursor = new Date(windowStart)
    while (cursor.getTime() + slotMs <= windowEnd.getTime()) {
      // Skip slots within the minimum notice window
      if (cursor > minNoticeCutoff) {
